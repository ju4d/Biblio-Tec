...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($data['criterio'], $data['busqueda'])) {
        ...
        $campo = ($criterio === 'nombre') ? 'titulo' : 'autor';

        // Preparar la consulta SQL para buscar los libros
        $sql = "SELECT l.id_libro, l.titulo, l.autor, l.edicion, l.precio, l.stock, g.nombre AS genero, p.nombre AS editorial
                FROM libros l
                JOIN genero g ON l.id_genero = g.id_genero
                JOIN proveedor p ON l.id_editorial = p.id_editorial
                WHERE l.$campo LIKE ?";
        ...
        require 'conectar.php';
            ...
            if ($stmt) {
                // Asociar parámetros
                $stmt->bind_param("i", $id); // Cambio aquí: usar $id en lugar de $nombre
