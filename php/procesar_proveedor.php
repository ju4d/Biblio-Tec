...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($data['nombre'], $data['telefono'], $data['direccion'])) {
        ...
        $sql_proveedor = "INSERT INTO proveedor ( telefono, direccion, nombre) VALUES (?, ?, ?)";
        $stmt_proveedor = $conexion->prepare($sql_proveedor);
        $stmt_proveedor->bind_param("sss",  $telefono, $direccion, $nombre);

        // Ejecutar la consulta
        if ($stmt_proveedor->execute()) {
            echo json_encode(['success' => true, 'message' => 'proveedor registrado con éxito.']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Error al registrar el proveedro en la base de datos: ' . $stmt_proveedor->error]);
        }
